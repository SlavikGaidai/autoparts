...
if (url.includes("=")) {
  ...
  if (category_query[0] == "category") {
    document.addEventListener("DOMContentLoaded", () => {
      ...
      product_heading.textContent = `Категорія : ${decodeURIComponent(category_query[1])
        .toUpperCase()
        .split("%20")
        .join(" ")}`;
      productRequest(`?category=${decodeURIComponent(category_query[1])}`);
    });
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      product_heading.textContent = `Search result for : '${decodeURIComponent(category_query[1])
        .split("%20")
        .join(" ")}'`;
...
async function productRequest(query = "?_limit=10") {
  ...
  if (data.length == 0 || data == null) {
    ...
    append.innerHTML = `<img style="height: 300px; display: block;" src="https://cdni.iconscout.com/illustration/premium/thumb/confusing-woman-due-to-empty-cart-4558760-3780056.png"><h3 style="color: rgb(255,142,85)">Нічого не знайдено</h3>`;
    return false;
  }
  productAppend(data);
}
